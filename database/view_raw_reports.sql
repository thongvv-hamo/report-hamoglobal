...
-- ----------------------------
-- View structure for view_raw_reports
-- ----------------------------
IF EXISTS (SELECT * FROM sys.all_objects WHERE object_id = OBJECT_ID(N'[dbo].[view_raw_reports]') AND type IN ('V'))
	DROP VIEW [dbo].[view_raw_reports]
GO

CREATE VIEW [dbo].[view_raw_reports] AS WITH TmpDV_TongTienThanhToan AS
	(SELECT CAST
		(SUM(CTTT.ThanhTienThanhToan) AS DECIMAL (18, 0)) AS TienThanhToan,
		 CAST(CTTT.NgayThanhToan AS DATE) as NgayThanhToan,
...
CTTDV_ThanhToanCongNo AS (
					...
					FROM
						...
						CROSS APPLY (
							...
							WHERE
								...
								AND TmpDV_TongTienThanhToan.NgayThanhToan =  CAST(CTTT.NgayThanhToan AS DATE)
						) AS ca
			...
			FROM (
					...
					CROSS APPLY (
							...
							WHERE
								...
								AND TmpSP_TongTienThanhToan.NgayThanhToan =  CAST(CTTT.NgayThanhToan AS DATE)
					) AS ca
	...
	SELECT -- hồ sơ hoàn huỷ DỊCH VỤ
		 CAST(CTTT.NgayThuChi AS DATE) AS 'DateOfApplication',
		DMPB.IDPhongBan AS 'site_id',
		 DMPB.TenPhongBan AS 'SiteName',
		N'Hoàn huỷ' AS 'Type',
